...
const CACHE_KEY_SNAPSHOT = 'agiarena-snapshot-cache'
const CACHE_KEY_META = 'agiarena-meta-cache'
...
export function useMarketSnapshotMeta() {
  return useQuery<SnapshotMetaResponse>({
    ...
    queryFn: async () => {
      try {
        const res = await fetchWithTimeout(`${API_URL}/data-node/snapshot/meta`, 10_000)
...
export function useMarketSnapshot() {
  return useQuery<SnapshotResponse>({
    ...
    queryFn: async () => {
      try {
        const res = await fetchWithTimeout(`${API_URL}/data-node/snapshot`, 45_000)
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const data = await res.json()
