  ...
  return (
    <div className="animate-pulse">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-center gap-4 p-4 border-b border">
...
function EmptyState() {
  return (
    <div className="p-8 text-center">
      <p className="text-secondary font-mono">No bets yet</p>
...
export function RecentBetsTable({ bets, isLoading = false }: RecentBetsTableProps) {
  ...
  return (
    <Table>
      ...
      <TableBody>
        {bets.map((bet) => {
          const resultColor = bet.result >= 0 ? 'text-green' : 'text-secondary'
          ...
          return (
            <TableRow key={bet.betId}>
              <TableCell>
                ...
                <span
                  className="text-secondary font-mono text-sm cursor-default"
                  title={bet.betId}
                >
              ...
              <TableCell className="font-mono text-secondary text-sm">
